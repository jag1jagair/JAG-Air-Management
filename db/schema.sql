create table if not exists owners (
  id uuid primary key default gen_random_uuid(),
  name text not null,
  email text,
  phone_e164 text,
  sms_opt_in boolean default true,
  notify_enabled boolean default true,
  created_at timestamptz default now()
);

create table if not exists aircraft (
  id uuid primary key default gen_random_uuid(),
  tail text unique not null,
  seats int not null,
  seat_sharing_enabled boolean not null default false,
  base_airport text not null,
  owner_pool text,
  created_at timestamptz default now()
);

create table if not exists ownerships (
  id uuid primary key default gen_random_uuid(),
  aircraft_id uuid references aircraft(id) on delete cascade,
  owner_id uuid references owners(id) on delete cascade,
  percentage numeric
);

create table if not exists owner_weeks (
  id uuid primary key default gen_random_uuid(),
  aircraft_id uuid references aircraft(id) on delete cascade,
  owner_id uuid references owners(id) on delete cascade,
  week_start date not null,
  week_end date not null,
  recurs text
);

create table if not exists passengers (
  id uuid primary key default gen_random_uuid(),
  owner_id uuid references owners(id) on delete set null,
  full_name text not null,
  dob date,
  passport_number text,
  passport_country text,
  visa_info text,
  created_at timestamptz default now()
);

create table if not exists flights (
  id uuid primary key default gen_random_uuid(),
  aircraft_id uuid references aircraft(id) on delete cascade,
  dep text not null,
  arr text not null,
  start_time timestamptz not null,
  end_time timestamptz not null,
  whole_aircraft boolean not null default true,
  status text not null default 'REQUESTED',
  policy_snapshot jsonb default '{}'::jsonb,
  us_stay_address text,
  created_at timestamptz default now()
);

create table if not exists flight_seats (
  id uuid primary key default gen_random_uuid(),
  flight_id uuid references flights(id) on delete cascade,
  owner_id uuid references owners(id) on delete cascade,
  seats_reserved int not null default 1
);

create index if not exists idx_aircraft_tail on aircraft(tail);
create index if not exists idx_flights_aircraft_start on flights(aircraft_id, start_time);

-- === App Users & RLS helpers ===
create table if not exists app_users (
  id uuid primary key default gen_random_uuid(),
  email text unique not null,
  role text not null check (role in ('Manager','Owner')),
  owner_id uuid references owners(id) -- null for Manager
);

-- Helper to get current email from JWT
create or replace function current_email() returns text language sql stable as $$
  select coalesce(nullif(auth.jwt()->>'email', ''), nullif(current_setting('request.jwt.claims', true)::json->>'email',''));
$$;

-- Is the current user a Manager?
create or replace function is_manager() returns boolean language sql stable as $$
  select exists (select 1 from app_users au where au.email = current_email() and au.role = 'Manager');
$$;

-- Get current user's owner_id (if any)
create or replace function current_owner_id() returns uuid language sql stable as $$
  select owner_id from app_users where email = current_email();
$$;

alter table owners enable row level security;
alter table aircraft enable row level security;
alter table ownerships enable row level security;
alter table flights enable row level security;
alter table flight_seats enable row level security;

-- Owners: self can read; manager can read all
drop policy if exists owners_read on owners;
create policy owners_read on owners
for select using (
  is_manager() or id = current_owner_id()
);

-- Aircraft: manager can read all; owners can read aircraft they have ownership in
drop policy if exists aircraft_read on aircraft;
create policy aircraft_read on aircraft
for select using (
  is_manager() or exists (
    select 1 from ownerships o
    where o.aircraft_id = aircraft.id
      and o.owner_id = current_owner_id()
  )
);

-- Ownerships: manager or rows tied to current owner
drop policy if exists ownerships_read on ownerships;
create policy ownerships_read on ownerships
for select using (
  is_manager() or owner_id = current_owner_id()
);

-- Flights: manager can do everything; owners can view flights on their aircraft; owners can insert requests on their aircraft
drop policy if exists flights_read on flights;
create policy flights_read on flights
for select using (
  is_manager() or exists (
    select 1 from ownerships o where o.aircraft_id = flights.aircraft_id and o.owner_id = current_owner_id()
  )
);

drop policy if exists flights_insert on flights;
create policy flights_insert on flights
for insert with check (
  is_manager() or exists (
    select 1 from ownerships o where o.aircraft_id = flights.aircraft_id and o.owner_id = current_owner_id()
  )
);

drop policy if exists flights_update on flights;
create policy flights_update on flights
for update using ( is_manager() );

drop policy if exists flights_delete on flights;
create policy flights_delete on flights
for delete using ( is_manager() );

-- Flight seats: manager can read; owners can read/insert their own seat reservations
alter table flight_seats enable row level security;
drop policy if exists flight_seats_read on flight_seats;
create policy flight_seats_read on flight_seats
for select using ( is_manager() or owner_id = current_owner_id() );

drop policy if exists flight_seats_insert on flight_seats;
create policy flight_seats_insert on flight_seats
for insert with check ( is_manager() or owner_id = current_owner_id() );

-- === Amend flights for priority & auditing ===
alter table flights add column if not exists requested_by_owner_id uuid references owners(id);
alter table flights add column if not exists inside_min_notice boolean default false;
alter table flights add column if not exists priority_flag text default 'NONE'; -- NONE | WEEK_OWNER_PRIORITY

-- Helper: find the owner_id for an email (manager has NULL)
create or replace function owner_id_for_email(e text) returns uuid language sql stable as $$
  select owner_id from app_users where email = e;
$$;

-- Helper: does this time fall within an owner's assigned week for the aircraft?
create or replace function week_owner_for_time(p_aircraft uuid, p_time timestamptz) returns uuid language sql stable as $$
  select ow.owner_id
  from owner_weeks ow
  where ow.aircraft_id = p_aircraft
    and p_time::date >= ow.week_start
    and p_time::date <= ow.week_end
  limit 1;
$$;

-- Trigger: before insert on flights, set requested_by_owner_id, inside_min_notice, and priority_flag
create or replace function flights_before_insert() returns trigger language plpgsql as $$
declare
  req_email text;
  req_owner uuid;
  wk_owner uuid;
  hours_until numeric;
begin
  -- who is requesting?
  req_email := current_email();
  req_owner := owner_id_for_email(req_email);
  new.requested_by_owner_id := req_owner;

  -- min notice calc
  hours_until := extract(epoch from (new.start_time - now()))/3600.0;
  new.inside_min_notice := (hours_until < 24);

  -- week owner priority calc (72h rule): if there exists a flight request from the week owner placed >=72h prior, mark priority
  wk_owner := week_owner_for_time(new.aircraft_id, new.start_time);
  if wk_owner is not null then
    -- If the requester is the week owner AND start_time is 72h away, mark priority
    if req_owner = wk_owner and hours_until >= 72 then
      new.priority_flag := 'WEEK_OWNER_PRIORITY';
    else
      new.priority_flag := 'NONE';
    end if;
  else
    new.priority_flag := 'NONE';
  end if;

  return new;
end;
$$;

drop trigger if exists trg_flights_before_insert on flights;
create trigger trg_flights_before_insert
before insert on flights
for each row execute function flights_before_insert();
